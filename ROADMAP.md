...
## Phase 3.5: 법안/표결 상세 페이지 — ❌ 미착수

> 목록 페이지만 있고 상세 페이지가 없어 사용자가 개별 법안/표결 정보를 확인할 수 없는 문제 해결

### 백엔드
- [ ] GET /api/bills/:id — 법안 상세 조회 (제목, 상태, 위원회, 발의자 목록)
- [ ] GET /api/votes/:id — 표결 상세 조회 (기존 member-votes 엔드포인트 활용/정리)

### 프론트엔드 — 표결 상세 (`/votes/[id]`)
- [ ] 표결 기본 정보 (법안명, 표결일, 결과, 위원회)
- [ ] 찬/반/기권/불참 비율 차트 (VoteResultBar 활용)
- [ ] 의원별 투표 목록 (정당별 그룹핑, 사진 + 이름 + 투표 결과)
- [ ] "좌석 배치도로 보기" 링크 → `/chamber?voteId=xxx`

### 프론트엔드 — 법안 상세 (`/bills/[id]`)
- [ ] 법안 기본 정보 (제목, 상태, 제안일, 위원회)
- [ ] 대표 발의자 + 공동 발의 의원 목록 (사진 + 이름 + 정당)
- [ ] 관련 표결 연결 (해당 법안의 본회의 표결이 있으면 링크)

### 페이지 간 연결
- [ ] 표결 목록 → 표결 상세 링크
- [ ] 법안 목록 → 법안 상세 링크
- [ ] 의원 상세 표결 탭 → 해당 표결 상세 링크
- [ ] 의원 상세 법안 탭 → 해당 법안 상세 링크
