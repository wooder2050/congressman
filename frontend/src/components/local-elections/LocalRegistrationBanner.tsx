    ...
    <section className="space-y-3 rounded-xl border border-(--color-primary) bg-blue-50 p-4 sm:p-5 dark:bg-blue-950/30">
      ...
      <div>
        ...
        <p className="mt-1.5 text-sm leading-relaxed text-(--color-text-secondary)">
          {stage === "campaign"
            ? "5월 21일 0시 공식 선거운동이 시작돼 6월 2일까지 13일간 진행됩니다. 5/28 0시부터 6/3 투표 마감까지 여론조사 공표가 금지되어 '깜깜이 선거' 국면에 진입합니다. 중앙선관위 공식 데이터 기준으로 광역단체장·기초단체장·광역의원·기초의원·교육감 전 선거구의 후보자 명단·경력·공약·재산·사진이 표시됩니다. 5/18 투표용지 인쇄 시한이 지나, 이후 사퇴해도 인쇄된 후보 이름은 그대로 표시됩니다(사표 위험)."
            : stage === "closed"
      ...
      <p className="text-xs text-(--color-text-tertiary)">
        ...
        도 함께 확인하세요. 평택을 5파전(김용남·조국·유의동·황교안·김재연), 부산 북갑(한동훈·박민식·하정우) 등 격전지에서 5/27 D-7 시점 단일화 협상이 사실상 중단된 채 다자 구도가 유지될 전망입니다.
      </p>
    </section>
  );
}
