...
import { AUDIT_2026, type AuditCommittee, type AuditSource } from "@/data/audit-2026";
...
/** "10-06" → 2026-10-06 00:00 KST의 날짜 키 */
function dayKey(md: string): string {
  return `${YEAR}-${md}`;
}
...
function formatDate(date: string): string {
  ...
  return last ? `${formatMd(first)}~${formatMd(last)}` : formatMd(first);
}

/** 오늘(KST) 기준 진행 상태 */
function auditStatus(now: Date): { label: string; tone: "upcoming" | "live" | "done" } {
...
function byDate(committees: AuditCommittee[]) {
  ...
  for (const c of committees) {
    for (const day of c.days) {
      const key = dayKey(day.date.split("~")[0]);
...
function Sources({ sources }: { sources: AuditSource[] }) {
    ...
    <div className="mx-auto max-w-4xl space-y-10">
      ...
      <header className="space-y-4">
        ...
        <div className="rounded-xl border border-(--color-border-primary) bg-(--color-bg-secondary) px-4 py-3 text-xs leading-relaxed text-(--color-text-secondary)">
          ...
          상임위별 일정은 해당 위원회의 계획서 채택을 보도한 기사로 확인한 것만 적고, 확인되지 않은
          위원회는 날짜를 비워 둡니다. 쟁점은 발언 주체를 밝혀 인용하고 여야 입장을 함께 싣습니다.
          국정감사 제도 설명은{" "}
          <Link
        ...
        <Sources sources={d.periodSources} />
      </header>
          ...
          {confirmed.map((c) => (
            ...
            >
              ...
              <ul className="mt-3 divide-y divide-(--color-border-primary) text-sm">
                {c.days.map((day) => (
                  <li key={`${c.short}-${day.date}`} className="flex gap-3 py-1.5">
      ...
      <section aria-labelledby="dates-title" className="space-y-3">
        ...
        <p className="text-sm text-(--color-text-secondary)">
          계획서가 확인된 위원회의 일정을 날짜순으로 모았습니다. 같은 날 여러 상임위가 동시에
          열립니다.
        </p>
          ...
          <ul className="divide-y divide-(--color-border-primary) text-sm">
            {liveSchedules.map((s) => (
              <li key={s.id} className="flex flex-wrap gap-x-3 py-2">
                ...
                <span className="text-(--color-text-secondary)">{s.agenda || s.title}</span>
              </li>
            ))}
          </ul>
        ) : (
